<template>
  <Page title="推荐词" class="RecommendKeywords">
    <Table :loading="state.loading" :list="state.list">
      ...
      <el-table-column :label="$l('关键词')" prop="keyword" />
      <el-table-column :label="$l('热度')" prop="popularity" width="100" />
    </Table>
  </Page>
</template>
<script setup>
...
// 挂载
onMounted(() => {
  let localKeyword = localStorage.getItem('RecommendKeywordsKeyword');
  if (localKeyword) {
...
const toSearch = () => {
  ...
  aim.unshift(keyword);
  aim = aim.slice(0,5);
  localStorage.setItem('RecommendKeywordsKeyword', aim.join(','));
...
const toRemoveKeyword = (v, i) => {
  ...
  localStorage.setItem(
    'RecommendKeywordsKeyword',
    state.localKeyword.join(','),
  );
};
// 卸载
</script>
