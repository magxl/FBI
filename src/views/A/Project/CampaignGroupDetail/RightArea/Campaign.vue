<template>
  <div class="RightAreaCampaign p8 pt0">
    <Card noshadow border="border-gray3">
      ...
      <div class="flexMode vs">
        <div class="statisticsArea flexMode flexWrap hb p8 pb0">
          <div
...
const initColor = () => {
  const color = JSON.parse(localStorage.getItem('projectCampaignColor')) || {};
  state.color = color;
};
const getStatistics = () => {
...
const setColor = (it, v) => {
  const { color } = state;
  color[it.label] = v;
  localStorage.setItem('projectCampaignColor', JSON.stringify(color));
};
const clearColor = (it) => {
  const { color } = state;
...
const getOffsetTop = () => {
  ...
  console.info('Campaign offset', top);
